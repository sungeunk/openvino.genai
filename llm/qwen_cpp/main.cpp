...
int main(int argc, char **argv) {
  try {
    ...
    if (args.convert_kv_fp16) {
        ...
        std::cout << "args.stateful: " << args.stateful << "\n";
        if (!args.stateful) {
            // Reshape model
        ...
        {
            std::cout << "Skip reshape model input for stateful model\n";
        }
        inputs = model->inputs();
    ...
    // Direct compile modifiled Qwen model with FP16 KV input & output + Reduced logits operations [1,1,151936]
        ...
        std::cout << "model_inputs.size() " << model_inputs.size() << "\n";
        int32_t out_token;
        int sentence_num = 0;
        ...
        for (std::string input_text : sentences) {
            ...
            if (args.stateful){
                // If stateful model contains 3 inputs (input_ids, attention_mask, beam_id), use WA to set beam_idx batch=1, value as 0 for greedy search case 
